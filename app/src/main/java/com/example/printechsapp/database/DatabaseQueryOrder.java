...
public class DatabaseQueryOrder {
    ...
    public DatabaseQueryOrder(Context context){
        this.context = context;
    }


    public void addItemOrder(ItemModel contact) {
        ...
        db.insert(Material_Recipt_Table,null,values);
        db.close();
    }

    public LinkedHashSet<String> getAllOrderTimeMillies() {
        LinkedHashSet<String> datalist = new LinkedHashSet<>();

        String selectQuery = "SELECT  TIME_MILLIES FROM " + Material_Recipt_Table;
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
    ...
    public ItemModel getSingleItemOrder(String timeMillies) {
        ArrayList<ItemModel> datalist = new ArrayList<>();

        String selectQuery = "SELECT  * FROM " + Material_Recipt_Table + " WHERE "
    ...
    public void updateTotalQTYCountOrder(ItemModel connect, String time_millies){
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        ContentValues values=new ContentValues();
        values.put(QTY, connect.getQTY());
        db.update(Material_Recipt_Table, values, TIME_MILLIES + " = ?",
                new String[] { (time_millies) });
    }

    public void addOrderDetails(OrderModel model) {
    ...
    public ArrayList<ItemModel> getAllOrderItems(String order_id) {
        ArrayList<ItemModel> datalist = new ArrayList<>();

        String selectQuery = "SELECT  * FROM " + Order_Items_Table + " WHERE " + Order_Id + " = " + order_id;
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        Cursor cursor = db.rawQuery(selectQuery, null);
        if (cursor.moveToFirst()) {
    ...
    public void insertSingleOrderItems(ItemModel item) {
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        ContentValues values = new ContentValues();
    ...
    public ArrayList<ItemModel> getSingleOrderItems(String order_id) {
        ArrayList<ItemModel> datalist = new ArrayList<>();
        String selectQuery = "SELECT  * FROM " + Order_Items_Table + " WHERE " + Order_Id + " = " + order_id;
    ...
    public String getOrderTotalQTY(String orderID) {
        String orderTotalQty = "0";
        String selectQuery = "SELECT  * FROM " + Order_Table + " WHERE " + Order_Id + " = " + orderID;
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        Cursor cursor = db.rawQuery(selectQuery, null);
        if (cursor.moveToFirst()) {
    ...
    public ArrayList<ItemModel> getMaterialItems(String timeMillies, String location) {
        ArrayList<ItemModel> datalist = new ArrayList<>();
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        Cursor cursor;
        if (!location.equals("0")){
            String query = "select * from " + Material_Recipt_Table +" where " + TIME_MILLIES + " = ? AND " + Key_Loc + " = ? ";
            cursor = db.rawQuery(query , new String[] { timeMillies,location});
        }else {
            String selectQuery = "SELECT * FROM " + Material_Recipt_Table + " where TIME_MILLIES = '" + timeMillies + "'";
    ...
    public void updateTotalQTYMaterial(ItemModel connect, String time_millies){
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        ContentValues values=new ContentValues();
        values.put(QTY, connect.getQTY());
    ...
    public void deleteMaterialItem(int id) {
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        db.delete(Material_Recipt_Table, Colunm_id + " = ?",
                new String[] { String.valueOf(id) });
        db.close();
    }

    public void updateMaterialItem(ItemModel connect, String column_id){
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        ContentValues values = new ContentValues();
        ...
        db.update(Material_Recipt_Table, values, Colunm_id + " = ?",
                new String[] { (column_id) });
    }

    // Get Single item Qty (Unit count)
    public int singleItemQTY(String barcode, String order_id){
        int itemQTY = 0;
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        String selectQuery = "select * from " + Order_Items_Table +" where " + Barcode + " = ? AND " + Order_Id + " = ? ";
    ...
    public int scannedItemQTY(String barcode, String order_id, String time_millies){
        int itemQTY = 0;
        DatabaseClass dbClass = new DatabaseClass(context);
        SQLiteDatabase db = dbClass.getWritableDatabase();
        String selectQuery2 = "select * from " + Material_Recipt_Table + " where " + Barcode + " = ? AND " + Order_Id + " = ? AND  " + TIME_MILLIES + " = ?";
        //String selectQuery = "select * from " + Material_Recipt_Table +" where " + Barcode + " = ? AND " + Order_Id + " = ? ";
